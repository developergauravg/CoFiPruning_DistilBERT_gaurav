...
def update_params(model, zs: Optional[Dict[str, torch.Tensor]]) -> None:
    ...
    if "head_z" in zs:
        for layer in range(num_layers):
            head_z = zs["head_z"][layer].to(device).squeeze().clone()
            ...
            if "head_layer_z" in zs:
                head_layer_z = zs["head_layer_z"][layer].to(device)
    ...
    if "hidden_z" in zs:
        hidden_z = zs["hidden_z"].to(device).squeeze().clone()
