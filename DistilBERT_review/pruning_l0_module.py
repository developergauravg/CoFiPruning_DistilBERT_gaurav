"""
Port of ``models/l0_module.py::L0Module`` from the official CoFiPruning
repository to DistilBERT.

Every piece of Hard-Concrete / L0 math below (``cdf_qz``, ``quantile_concrete``,
``_sample_z``, ``_deterministic_z``, the Lagrangian sparsity regularizer, and
``calculate_model_size``) is a direct, unmodified port of the official
implementation. The only thing that changes is which config attributes feed
the parameter-counting formulas, because DistilBERT names its dimensions
differently than BERT:

    BERT field              DistilBERT field
    ----------              ----------------
    hidden_size             dim
...
DistilBERT also has no pooler and no token-type embeddings, but neither of
those affects the L0 module itself (the official L0Module never references
the pooler or token-type embeddings either) so no other changes are needed.

This module intentionally keeps the same ``zs`` dict keys as the official
BERT L0Module (``hidden_z``, ``head_z``, ``head_layer_z``, ``intermediate_z``,
``mlp_z``) so that ``modeling_distilbert.py`` and ``cofi_utils.py`` can stay
close, reviewable ports of their BERT counterparts instead of diverging into
DistilBERT-specific naming.
"""
...
# Hard Concrete distribution constants, identical to the official module.
limit_a, limit_b, epsilon = -0.1, 1.1, 1e-6
logger = logging.get_logger(__name__)


class DistilBertL0Module(Module):
    def __init__(
        self,
        config,
        ...
        magical_number: float = 0.8,  # from Wang et al. 2020, same as official module
    ):
        super().__init__()
        self.all_types = ["hidden_z", "intermediate_z", "mlp_z", "head_layer_z", "head_z"]
        self.pruning_type = pruning_type

        # --- DistilBERT dimension fields (see module docstring for the BERT mapping) ---
        ...
        # q_lin, k_lin, v_lin, out_lin: four (dim x dim) linears + four dim-sized biases,
        # exactly analogous to BERT's query/key/value/attention.output.dense.
        ...
        # lin1 (dim -> hidden_dim) + lin2 (hidden_dim -> dim), analogous to
        # BERT's intermediate.dense/output.dense.
        ...
        # we ignore the parameters in normalization layers (they take a very small amount)
        ...
        logger.info("********** Initializing DistilBERT L0 Module **********")
    ...
    def transform_scores_for_head(self):
        ...
        if "head_layer" in self.types:
            ...
            all_head_score = None
        head_score = 1 - self.cdf_qz(0, self.head_loga)  # n_layers * n_heads

        if all_head_score is not None:
            all_head_score = all_head_score.view(-1, 1, 1)  # n_layers * 1 * 1
        head_score = head_score.unsqueeze(-1)  # n_layers * n_heads * 1

        return all_head_score, head_score

    def get_num_parameters_for_mlp(self):
        intlayer_score = 1 - self.cdf_qz(0, self.intlayer_loga)  # n_layers
        int_score = 1 - self.cdf_qz(0, self.int_loga)  # n_layers * hidden_dim
    ...
    def get_num_parameters_and_constraint_for_hidden(self):  #! calculate the current sparsity
        ...
        hidden_score = 1 - self.cdf_qz(0, self.hidden_loga)  # dim
    ...
    def lagrangian_regularization(self, pruned_steps):
        ...
        lagrangian_loss = (  #! see CoFi paper appendix
    ...
    def calculate_model_size(self, zs):
        ...
        remaining_intermediate_nums = (
            intermediate_z.reshape(self.num_hidden_layers, self.intermediate_size).sum(-1).tolist()
        )
